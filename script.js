...
// =========================
// PATCH: Templates module (Nav + Page + Listing)
// Paste near the bottom of script.js, right before the final "})();"
// =========================
(() => {
  if (window.__SINNERS_TEMPLATES_PATCH__) return;
  window.__SINNERS_TEMPLATES_PATCH__ = true;
  ...
  function isAllowed(name) {
    ...
    return ["pdf","doc","docx","xls","xlsx","csv","txt","png","jpg","jpeg","webp"].includes(ext);
  }
  ...
  function injectNavButton() {
    ...
    if (nav.querySelector('[data-route="templates"]')) {
      return nav.querySelector('[data-route="templates"]');
    }

    const utilsBtn = nav.querySelector('[data-route="utils"]');
    const btn = document.createElement("button");
    btn.className = "nav-item";
    btn.dataset.route = "templates";
    btn.innerHTML = `📁 <span>Resources</span>`;

    if (utilsBtn && utilsBtn.nextSibling) {
      utilsBtn.insertAdjacentElement("afterend", btn);
    } else {
      nav.appendChild(btn);
    }
    return btn;
  ...
  // Inject Page
  // -------------------------
  function injectTemplatesPage() {
    const content = document.querySelector(".content");
    if (!content) return null;

    let page = document.getElementById("page-templates");
    if (page) return page;
    ...
    page.innerHTML = `
      <div class="page-head">
        <h2>Resources</h2>
        <div class="row gap">
          <button class="btn ghost" id="btnRefreshTemplates">Refresh</button>
        </div>
      </div>

      <div class="card">
        <div class="row gap" style="margin-bottom:10px;">
          <input id="tplSearch" placeholder="Search templates by name…" />
          <span class="tiny muted" id="tplModeBadge" style="white-space:nowrap;"></span>
        </div>
        <div class="divider"></div>
        <div id="templatesList" class="listbox"></div>
        <p class="tiny muted" style="margin-top:12px;">
          Workspace mode lists files from your selected workspace <code>templates/</code> folder.
          GitHub Pages mode uses <code>/templates/manifest.json</code>.
        </p>
      </div>
    `;
  ...
  // -------------------------
  // Show page helper (works even if your internal router doesn't know "templates")
  // -------------------------
  function showTemplatesRoute() {
    // Hide all pages
    $$(".page").forEach(p => p.hidden = true);

    // Set nav active state
    $$(".nav-item").forEach(b => b.classList.toggle("active", b.dataset.route === "templates"));
  ...
  // -------------------------
  // Load templates (Workspace OR GitHub)
  // -------------------------
  async function loadFromWorkspace() {
    // expects global Store + Workspace
    const root = Store?.state?.rootHandle;
    if (!root) return [];

    const dir = await Workspace.dir(root, "templates");
  ...
  async function loadFromManifest() {
    // GitHub Pages can’t list folders, so we use manifest.json
    try {
      const res = await fetch("templates/manifest.json", { cache: "no-store" });
      if (!res.ok) return [];
      const json = await res.json();
      const files = Array.isArray(json.files) ? json.files : [];
      return files.filter(isAllowed).map(name => ({ name, size: null }));
    } catch {
      return [];
    }
  }
  ...
  function renderList(items) {
    ...
    for (const f of filtered) {
      ...
      const top = document.createElement("div");
      top.textContent = f.name;

      const meta = document.createElement("div");
      meta.className = "meta";
      meta.appendChild(document.createElement("span")).textContent = extOf(f.name).toUpperCase() || "FILE";
      if (f.size != null) meta.appendChild(document.createElement("span")).textContent = humanSize(f.size);
      ...
      // Click handlers depend on storage mode
      const mode = Store?.state?.mode;

      if (mode === "workspace") {
        openBtn.onclick = async () => {
          try {
            const dir = await Workspace.dir(Store.state.rootHandle, "templates");
            ...
            Toasts?.push?.({ title: "Templates", message: `Open failed: ${String(e?.message || e)}` });
          }
        };
        ...
        dlBtn.onclick = async () => {
          try {
            ...
            Toasts?.push?.({ title: "Templates", message: `Download failed: ${String(e?.message || e)}` });
          }
        };
      } else {
        const url = `templates/${encodeURIComponent(f.name)}`;
        openBtn.onclick = () => safeOpen(url);
        dlBtn.onclick = () => {
          const a = document.createElement("a");
          a.href = url;
          a.download = f.name;
  ...
  async function refreshTemplates() {
    const badge = document.getElementById("tplModeBadge");
    const mode = Store?.state?.mode;

    if (badge) {
      badge.textContent =
        mode === "workspace"
          ? "Mode: Workspace (shared folder)"
          : "Mode: GitHub (manifest.json)";
    }

    let items = [];
    try {
      if (mode === "workspace") items = await loadFromWorkspace();
      else items = await loadFromManifest();
    } catch (e) {
      Toasts?.push?.({ title: "Templates", message: `Load error: ${String(e?.message || e)}` });
    }
  ...
  // Wire up
  // -------------------------
  const btn = injectNavButton();
  injectTemplatesPage();

  // If nav button exists, hook it
  if (btn) {
    btn.addEventListener("click", async () => {
      showTemplatesRoute();
  ...
  document.addEventListener("click", async (e) => {
    if (e.target && e.target.id === "btnRefreshTemplates") {
      await refreshTemplates();
    }
  });

  document.addEventListener("input", (e) => {
    if (e.target && e.target.id === "tplSearch") {
      // re-render based on current list by reloading quickly
      refreshTemplates();
    }
  });
